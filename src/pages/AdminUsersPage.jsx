...
function UserForm({ user, submitting, onCancel, onSubmit }) {
  ...
  return (
    ...
    >
      ...
      {!user && (
        <div className="space-y-3 rounded-xl bg-blue-50 p-4 text-sm font-semibold leading-6 text-blue-800">
          <label className="flex cursor-pointer items-start gap-3">
            <input type="radio" name="accessMethod" value="temporary-password" checked={form.accessMethod === 'temporary-password'} onChange={(event) => setField('accessMethod', event.target.value)} className="mt-1.5 accent-emerald-700" />
            <span><strong className="block">Contraseña temporal (recomendado)</strong>Se muestra una sola vez al administrador y la persona deberá cambiarla en su primer ingreso.</span>
          </label>
          <label className="flex cursor-pointer items-start gap-3 border-t border-blue-100 pt-3">
            <input type="radio" name="accessMethod" value="email" checked={form.accessMethod === 'email'} onChange={(event) => setField('accessMethod', event.target.value)} className="mt-1.5 accent-emerald-700" />
            <span><strong className="block">Invitación por correo</strong>Supabase enviará un enlace para que la persona active la cuenta y cree su contraseña.</span>
          </label>
        </div>
      )}
...
export default function AdminUsersPage() {
  ...
  const counts = useMemo(() => ({
    total: users.length,
    active: users.filter((user) => user.isActive && user.accountStatus === 'active').length,
  ...
  const saveUser = async (form) => {
    ...
    try {
      ...
      const payload = await readPayload(await fetchWithRetry(url, {
        attempts: 1,
      ...
      if (payload.temporaryPassword) {
        setTemporaryAccess({ email: form.email, password: payload.temporaryPassword })
        setMessage('Usuario creado. Copia ahora los datos de acceso temporal; la contraseña no volverá a mostrarse.')
      } else {
  ...
  return (
    <main className="min-h-[calc(100vh-4.5rem)] bg-slate-50 px-4 py-8 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        {temporaryAccess && (
          <section className="mt-4 rounded-2xl border border-amber-200 bg-amber-50 p-5 text-sm text-slate-800">
            <p className="font-black text-amber-900">Acceso temporal · visible una sola vez</p>
            <p className="mt-3"><span className="font-bold">Correo:</span> {temporaryAccess.email}</p>
            <p className="mt-1 break-all"><span className="font-bold">Contraseña:</span> <code className="rounded bg-white px-2 py-1 font-black">{temporaryAccess.password}</code></p>
            <button type="button" onClick={() => navigator.clipboard.writeText(`Correo: ${temporaryAccess.email}\nContraseña temporal: ${temporaryAccess.password}`)} className="mt-4 rounded-xl bg-amber-600 px-4 py-2 font-black text-white hover:bg-amber-500">Copiar datos de acceso</button>
          </section>
        )}
                  ...
                  {users.map((user) => (
                    <tr key={user.id} className="transition hover:bg-slate-50/70">
                      ...
                      <td className="px-6 py-5">
                        <span className={`inline-flex items-center gap-2 text-xs font-black ${!user.isActive ? 'text-rose-600' : user.accountStatus === 'invited' ? 'text-amber-700' : user.accountStatus === 'confirmed' ? 'text-sky-700' : 'text-emerald-700'}`}>
                          <span className={`size-2 rounded-full ${!user.isActive ? 'bg-rose-500' : user.accountStatus === 'invited' ? 'bg-amber-500' : user.accountStatus === 'confirmed' ? 'bg-sky-500' : 'bg-emerald-500'}`} />
                          {!user.isActive ? 'Deshabilitado' : user.accountStatus === 'invited' ? 'Invitación pendiente' : user.accountStatus === 'confirmed' ? 'Activado · sin ingreso' : 'Activo'}
                        </span>
                      </td>
                      ...
                      <td className="px-6 py-5"><div className="flex justify-end gap-2"><button onClick={() => sendPasswordReset(user)} disabled={user.accountStatus === 'invited'} title={user.accountStatus === 'invited' ? 'Debe aceptar primero la invitación' : 'Enviar enlace para restablecer contraseña'} className="grid size-9 place-items-center rounded-lg border border-slate-200 text-slate-500 hover:border-amber-200 hover:bg-amber-50 hover:text-amber-700 disabled:cursor-not-allowed disabled:bg-slate-50 disabled:text-slate-300" aria-label={`Restablecer contraseña de ${user.fullName}`}><KeyRound className="size-4" /></button><button onClick={() => { setShowCreate(false); setEditingUser(user); setError(''); setMessage(''); window.scrollTo({ top: 0, behavior: 'smooth' }) }} className="inline-flex items-center gap-2 rounded-lg border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800"><Pencil className="size-3.5" /> Editar</button></div></td>
                    </tr>
                  ))}
                </tbody>
              </table>
